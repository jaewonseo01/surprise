...
# Swin implementation from ViTST doesn't use the drop_path in SwinStage;
# Therefore deleted, to mitigate meta-tensor issues
...
class SwinEncoder(nn.Module):
    def __init__(self, config, grid_size):
        ...
        self.layers = nn.ModuleList(
            [
                SwinStage(# SwinStage implementation from original ViTST code doesn't use the "drop_path"
                    config=config,
...
class SwinPreTrainedModel(PreTrainedModel):
    ...
    def get_head_mask(self, head_mask, num_hidden_layers: int, is_attention_chunked: bool = False):
        """
        Minimal compatibility shim for HF Trainer/Accelerate.
        Swin doesn't use head masking in this code path, but we keep the interface.
        Returns a list/None structure compatible with downstream calls.
        """
        if head_mask is None:
            return [None] * num_hidden_layers

        # Accept: [num_heads] or [num_layers, num_heads]
        if isinstance(head_mask, (list, tuple)):
            head_mask = torch.tensor(head_mask, dtype=torch.float32, device=self.device)
        ...
        if head_mask.dim() == 1:
            ...
            if head_mask.size(0) != num_hidden_layers:
                raise ValueError(
                    f"head_mask 1st dim must be num_hidden_layers={num_hidden_layers}, got {head_mask.size(0)}"
                )
        else:
            raise ValueError(f"head_mask must have dim 1 or 2, got {head_mask.dim()}")

        # Convert to list of per-layer head masks
        # Shape each: [num_heads] 
